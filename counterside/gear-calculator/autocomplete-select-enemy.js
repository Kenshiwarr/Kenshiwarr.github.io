...
/*       for (let i = 0; i < checkSubs.length; i++) {
      ...
      $('#target-eva').html('<h>EVA: </h><span class="current_stats">' + Math.round(target_eva) + '</h> <span class="added_stats"> ' + ((target_eva-target_data[7]) > 0 ? ('(+' + Math.round(target_eva-target_data[7]) + ')'):'') + '('+ (chanceToDodge*100).toFixed(2) +'%)</span></span>');
